...
expect_inherits(laura_matrix, "matrix")
expect_inherits(laura_df, "data.frame")
expect_inherits(phy_dnabin, "DNAbin")
expect_inherits(phy_aabin, "AAbin")
expect_inherits(phy_align, "alignment")
expect_inherits(as.phyDat(laura_matrix), "phyDat")
expect_inherits(as.phyDat(laura_df), "phyDat")
expect_inherits(as.phyDat(phy_dnabin), "phyDat")
expect_equal(as.phyDat(phy_aabin), chloroplast)
expect_inherits(phyDat(laura_vec_dna), "phyDat")
expect_inherits(phyDat(laura_vec_user, type = "USER", levels = c("0","1")),
                    "phyDat")
expect_inherits(as.phyDat(phy_vec_factor), "phyDat")
expect_inherits(as.phyDat(phy_dnabin), "phyDat")
expect_inherits(as.phyDat(phy_align), "phyDat")
expect_inherits(c2d <- codon2dna(codon_align), "phyDat")
...
expect_inherits(subset_1 <- subset(Laurasiatherian, select = 1:1000,
                                 site.pattern = FALSE), "phyDat")
expect_inherits(subset_2 <- subset(Laurasiatherian, select = 1001:3179,
                                 site.pattern = FALSE), "phyDat")
expect_inherits(lauraCbind1 <- cbind(subset_1, subset_2), "phyDat")
expect_equal(baseFreq(lauraCbind1), baseFreq(Laurasiatherian))
expect_inherits(subset_3 <- subset(Laurasiatherian, select = 1:100,
                                        site.pattern = TRUE), "phyDat")
expect_inherits(subset_4 <- subset(Laurasiatherian, select = 101:1605,
                                        site.pattern = TRUE), "phyDat")
expect_equal(subset_1, Laurasiatherian[, 1:1000])
expect_error(subset(Laurasiatherian, 1:100), "subscript out of bounds")
expect_inherits(lauraCbind2 <- cbind(subset_3, subset_4), "phyDat")
expect_equal(baseFreq(lauraCbind2), baseFreq(Laurasiatherian))

ind <- sample(47, 10)
expect_inherits(subset_5 <- subset(Laurasiatherian, ind), "phyDat")
expect_inherits(subset_6 <- Laurasiatherian[ind,], "phyDat")
expect_equal(subset_5, subset_6)
expect_inherits(subset_7 <- subset(Laurasiatherian, ind, compress=TRUE),
                     "phyDat")
expect_true(object.size(subset_7) <= object.size(subset_5))

# test read and write
write.phyDat(Laurasiatherian, "tmp1.txt")
expect_inherits(laura <- read.phyDat("tmp1.txt"), "phyDat")
expect_equal(laura, Laurasiatherian)
unlink("tmp1.txt")

write.phyDat(Laurasiatherian, "tmp1.nex", format = "nexus")
expect_inherits(laura <- read.phyDat("tmp1.nex", format="nexus"),
                     "phyDat")
expect_equal(laura, Laurasiatherian)
unlink("tmp1.nex")

write.phyDat(chloroplast, "tmp2.txt")
expect_inherits(chloro <- read.phyDat("tmp2.txt", type="AA"), "phyDat")
expect_equal(chloro, chloroplast)
unlink("tmp2.txt")

write.phyDat(chloroplast, "tmp.fas", format="fasta")
expect_inherits(chloro_fas <- read.phyDat("tmp.fas", type="AA",
                                              format = "fasta"), "phyDat")
expect_equal(chloro_fas, chloroplast)
unlink("tmp.fas")

write.phyDat(chloroplast, "tmp2.nex", format="nexus")
expect_inherits(chloro_nex <- read.phyDat("tmp2.nex", type="AA",
                                               format = "nexus"), "phyDat")
expect_equal(chloro_nex, chloroplast)
unlink("tmp2.nex")

write.phyDat(user_align, "tmp3.txt")
expect_inherits(user2 <- read.phyDat("tmp3.txt", type="USER",
                              levels=c(0:9,"-"), ambiguity="?"), "phyDat")
expect_equal(user2, user_align)
unlink("tmp3.txt")

write.phyDat(user_align, "tmp3.fas", format="fasta")
expect_inherits(user_fas <- read.phyDat("tmp3.fas", type="USER",
                levels=c(0:9,"-"), ambiguity="?", format = "fasta"), "phyDat")
expect_equal(user_fas, user_align)
unlink("tmp3.fas")

write.phyDat(user_align, "tmp3.nex", format="nexus")
expect_inherits(user_nex <- read.phyDat("tmp3.nex", type="STANDARD",
                                               format = "nexus"), "phyDat")
...
expect_null(map2)
expect_inherits(map1, "data.frame")
