...
The advantage of Neighbor-Net is that it returns always a circular split system which can be always displayed in a planar (2D) graph. For planar graphs we can also plot only the outline (@Bagci2021).  

```{r outline, eval=TRUE, out.width = "90%", fig.dim = c(9, 4), fig.cap="NeighborNet showing a) all edges and b) only the outline."}
par("mar" = rep(1, 4), "mfrow" = c(1, 2))
plot(nnet, main="a)")
