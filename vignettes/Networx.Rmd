...
Splits graphs or phylogenetic networks are a useful way to display conflicting data or to summarize different trees. Here, we present two popular networks, consensus networks [@Holland2004] and Neighbor-Net [@Bryant2004;  @Bryant2023].                                  
...
```{r, eval=TRUE, fig.cap="Tree with bootstrap values and consensusNet for the same trees", out.width = "90%", fig.dim = c(7, 4)}
set.seed(1)
...
The advantage of Neighbor-Net is that it returns always a circular split system which can be always displayed in a planar (2D) graph. For planar graphs we can also plot only the outline (@Bagci2021) as in figure \@ref(fig:outline) c). 

```{r outline, eval=TRUE, out.width = "90%", fig.dim = c(9, 4), fig.cap="NeighborNet showing a) all edges, without using edge length information b) and to contrast only the outline."}
