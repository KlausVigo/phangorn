...
```
The function most users want to use infer phylogenies with MP is `pratchet`, an implementationof the parsimony ratchet [@Nixon1999]. This allows to escape local optima and find better trees than performing only NNI / SPR rearrangements.
...
```
Next we assign branch length to the tree. The branch length are proportional to the number of substitutions.
```{r acctran}
treeRatchet  <- acctran(treeRatchet, primates)
```
And remove edge lengths which are zero. 
```{r di2multi}
...
mt[order(mt$BIC),]
# choose best model from the table according to BIC (default)
fitStart <- as.pml(mt)
# assume the model GTR + G + I
