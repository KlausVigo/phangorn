...
```{r midpoint, fig.cap="MP phylogeny with bootstrap values."}
...
load("Trees.RData")
```
```{r mt_selected, echo=TRUE, eval=TRUE, cache=TRUE}
...
plotBS(midpoint(fit_mt$tree), p = 0.5, type="p", digits=2, main="Ultrafast bootstrap")
...
fit_strict <- pml_bb(primates, model="HKY+G(4)", method="ultrametric",
                     rearrangement="NNI")
```
```{r plot_strict_primates, fig.cap="Ultrametric ML phylogeny."}
plot(fit_strict)
```
...
```{r tipdated_plot, fig.cap="Tip dated ML phylogeny with time scale in years."}
