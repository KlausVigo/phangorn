...
BIC(fitGTR)
```
An alternative is to use the function `modelTest`  to compare different nucleotide or protein models the AIC, AICc or BIC, similar to popular programs ModelTest and ProtTest [@Posada1998], [@Posada2008], [@Abascal2005]. By default available nucleotide or amino acid models available are compared. 
...
Additionally we can also estimate tipdated phylogenies. Now we use as an example a H3N2 virus data set from treetime [@treetime]. Additionally to the alignment we also need to read in data containing the dates of the tips. 
```{r}
fdir <- system.file("extdata/trees", package = "phangorn")
tmp <- read.csv(file.path(fdir,"H3N2_NA_20.csv"))
H3N2 <- read.phyDat(file.path(fdir,"H3N2_NA_20.fasta"), format="fasta")
```
We use the internal function `candidate_tree` to compute a fast a starting tree with the constraints given by the dates.
We than can use `optim.pml` to optimize the tree and parameters.  
```{r}
dates <- setNames(tmp$numdate_given, tmp$name)
tree <- candidate_tree(H3N2, "tipdated", tip.dates=dates)
fit_td <- pml(tree, H3N2, rate=attr(tree, "rate")) 
fit_td <- optim.pml(fit_td, model="GTR", rearrangement="NNI", optRooted=TRUE, 
...
In unrooted trees we estimate one edge length parameter for each tree, for ultrametric trees we estimate only a parameter for each internal node and for tipdatedd trees we have one additional parameter for the rate. 
