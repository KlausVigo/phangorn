...
```
Here we set the minimum iteration of the parsimony ratchet to 100 iterations. As the ratchet performed implicitly bootstrap resampling we already computed some branch support, in out case with at least 100 bootstrap iterations. The function may returns several best trees, but these trees have no branch length assigned to them yet. 
```{r acctran}
...
If treeRatchet is a list of tree, an object of class `multiPhylo` we can subset the i-th trees with `treeRatchet[[i]]`.
