...
fitGTR 
@
With the control parameters the threshols for the fitting process can be changed. Here we want just to supress output during the fitting process. For larger trees the NNI rearrangements often get stuck in local maxima. We added a stochatic algortithm similar as in \cite{Nguyen2015}, which makes random NNI permuation to the tree, which than gets optimised to escape local optima. While this may find better trees it will also take more time.      
<<echo=TRUE>>=
fitGTR <- optim.pml(fitGTR, model="GTR", optInv=TRUE, optGamma=TRUE, 
...
\newpage
\section{Appendix: Standard scripts for nucleotide analysis}\label{sec:Appendix2}
...
\newpage
\section{Appendix 2: Standard scripts for amino acid analysis}\label{sec:Appendix1}
