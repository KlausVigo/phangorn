...
```{r future_bootstrap, eval = TRUE}
library(phangorn)
library(future)

data("Laurasiatherian")
tree <- upgma(dist.ml(Laurasiatherian))
plan(multisession, workers = 2)
bs_trees <- bootstrap.phyDat(Laurasiatherian,
     FUN=function(x)upgma(dist.ml(x)), bs=100)
plan(sequential)
tree <- plotBS(tree, bs_trees, "phylogram")
```
...
It is good practice to clean up and set the back-end to sequential after using parallel processing.  
