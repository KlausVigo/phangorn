...
The \Rfunction{seqLogo} function from the \Rpackage{seqLogo} package from Bioconductor provides a neat way to show proportions of a nucleotides of ancestral states (see figure \ref{fig:seqLogo}). You can install \Rpackage{seqLogo} the following way
<<eval=FALSE>>=
source("https://bioconductor.org/biocLite.R")
biocLite("seqLogo")
@

<<eval=FALSE>> %label=plotLOGO,include=FALSE
library(seqLogo)
