...
The are two differences plots the marginal distribution, we can either plot a specific site (17) 
```{r plotML, fig.cap="Fig 4. Ancestral reconstruction the using the maximum likelihood."}
plotAnc(anc.ml, 17)
title("ML")
```
or the sequence at a certain node:
```{r seqLogo_ML, fig.cap="Fig 1. Ancestral reconstruction for a node.", eval=TRUE}
plotSeqLogo(anc.ml, node="Node1", 1, 20)
```

We can also plot the joint distribution as a heatmap. The heatmap function in phangorn allows not only to display characters at the tips but also the estimates for all internal nodes.  
```{r anc_heatmap}
op <- par(mar=c(4,1,4,5))
anc_heatmap(anc.ml, select=1:50, cex=0.8, use.edge.length = T)
par(op)
```
...
This is a common problem in phylogentic comparative methods and we can use the function *ace* in the ape [@Paradis2019], *fitDiscrete* in the geiger [@Pennell2014] or *fitMK* in the phytools [@Revell2012] package. Here we want to show how to fit these models using *optim.pml*.
...
```{r SYM_reconstruction, fig.cap="Marginal reconstruciton for comparative data example."}
