...
# end new code
  ...
  tree <- NULL
  mp <- Inf
  if (is.null(start) && perturbation != "random_addition"){
  ...
  on.exit({
    ...
    if (any(search_history)){
      if (search_history[[1]]) class(start_trees) <- "multiPhylo"
      if (search_history[[2]]) class(search_trees) <- "multiPhylo"
