...
write.ancestral <- function(x, file="ancestral"){
  ...
  write.phyDat(x$state, file=paste0(file, "_state.fasta"), format="fasta")
  tab <- as.data.frame(x)

  tsv_file <- paste0(file, "_state.tsv.gz")
  tname <- paste0(file, "_tree.nwk")

  prop_name <- gzfile(tsv_file, "w")
  ...
  cat("# vignette(\"Ancestral\", package=\"phangorn\") in R.\n#\n",
      file = prop_name, append=TRUE)
  cat(colnames(tab), "\n", sep="\t", file = prop_name, append=TRUE)
  write.table(tab, file=prop_name, quote=FALSE, row.names=FALSE, col.names=FALSE,
              sep="\t", append=TRUE)
  close(prop_name)
...
#' @rdname write.ancestral
#' @export
as.ancestral <- function(tree, prob, align, ...){
