support <- function(tree, trees, method="FBP", tol=1e-8, scale=TRUE){
  ...
  if(method=="FBP" || method=="MCC"){
    ...
    if(method=="MCC" && any(!is.rooted(trees)))
      stop("All trees need to be rooted for method 'MCC'!")
    if (method=="FBP" && any(is.rooted(trees))) trees <- unroot(trees)
