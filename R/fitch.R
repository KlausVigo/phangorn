...
#    .Call("FNALL_NNI", as.integer(nr), node, edge, as.integer(n), as.integer(m), as.integer(m2), as.integer(root0))
    .Call("FNALL_NNI", as.integer(nr), node, edge, as.integer(n), as.integer(m), as.integer(m2))
}   
...
#    .Call("FNALL5", as.integer(nr), node, edge, as.integer(n), as.integer(m), as.integer(m2), as.integer(root0))
    .Call("FNALL5", as.integer(nr), node, edge, as.integer(n), as.integer(m), as.integer(m2))
}   
...
bab <- function (data, tree = NULL, trace = 1, ...) 
{
    ...
    visited1 = integer(nTips)
    visited2 = integer(nTips)
...
# index L[i] is length(node) tree mit i+1 
    ...
    while (npsc > 0) {
        ...
        visited1[a+1L] = visited1[a+1L] + 1L           
        ms = min(score)
        if(ms<=bound){
            if((a+1L)<nTips){
                ...
                l = length(ind)
                visited2[a+1L] = visited2[a+1L] + 1L           
    ...
    attr(result, "visited") = cbind(visited1, visited2)
    class(result) <- "multiPhylo"
    return(result)
}


# os raus, rbind, raus [] always delete last line, compressSites + mms (???) (combine fnodes + triplet)
    ...
    while (npsc > 0) {
        ...
        if(ms<=bound){
            if((a+1L)<nTips){
                ...
                ind = (1:L[a])[score<=bound]
                trees[[a+1]] <- .Call("AddOnes", tmpTree, as.integer(inord[a+1L]), as.integer(ind), as.integer(L[a]), as.integer(M[a])) 
                l = length(ind)
                visited2[a+1L] = visited2[a+1L] + 1L           
                ...
                ind = which(score==ms) 
                tmp <- vector("list", length(ind)) 
                tmp <- .Call("AddOnes", tmpTree, as.integer(inord[a+1L]), as.integer(ind), as.integer(L[a]), as.integer(M[a]))
                visited2[a+1L] = visited2[a+1L] + 1L           
